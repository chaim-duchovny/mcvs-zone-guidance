# filename: draughts_mcvs.py
"""
DRAUGHTS MCVS - Complete implementation analogous to breakthrough_mcvs.py

This file is fully adapted for Draughts (English/American Checkers).
Key features:
- Standard 8x8 board with 12 pieces per player on dark squares.
- Mandatory captures (if any capture available, only captures are legal).
- Single-jump captures only (no forced multi-jump chains – keeps action space 64×64 = 4096 to match the policy head).
- Kings move/capture in both directions.
- Promotion on reaching opponent's back row.
- Weighted matrix distinguishes men and kings via different δ₂ values.
- All other components (networks, zone DB, training, self-play, tournament interface) are identical to the Breakthrough version.

NOTE: MCVSSearcher and UCTSearcher implementations are included here as complete, functional versions based on standard AlphaZero-style MCTS.
They are game-agnostic and work with any game implementing the required interface.
"""
